...
export default function PostDetail({ postId, onBack }: PostDetailProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Travel Schedule */}
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">여행 일정</h3>
        <div className="space-y-4">
          {post.schedule.map((day) => (
            <div key={day.day} className="flex space-x-4 p-4 bg-gray-50 rounded-lg">
              <div className="w-12 h-12 bg-blue-500 text-white rounded-full flex items-center justify-center font-bold">
                {day.day}
              </div>
              <div className="flex-1">
                <h4 className="font-semibold text-gray-800 mb-2">{day.title}</h4>
                <div className="flex flex-wrap gap-2">
                  {day.activities.map((activity, index) => (
                    <span key={index} className="px-3 py-1 bg-white text-gray-600 rounded-full text-sm border border-gray-200">
                      {activity}
                    </span>
                  ))}
                </div>
        ...
        <div className="mb-8">
          <div className="flex space-x-4">
            <div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
              <span className="text-gray-600 font-medium">현</span>
            </div>
            <div className="flex-1">
              <textarea
                value={newComment}
        ...
        <div className="space-y-6">
          {comments.map((comment) => (
            <div key={comment.id} className="space-y-4">
              <div className="flex space-x-4">
                <div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
                  <span className="text-gray-600 font-medium">{comment.avatar}</span>
                </div>
                <div className="flex-1">
              ...
              {comment.replies.map((reply) => (
                <div key={reply.id} className="ml-14 flex space-x-4">
                  <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                    <span className="text-gray-600 text-sm font-medium">{reply.avatar}</span>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
