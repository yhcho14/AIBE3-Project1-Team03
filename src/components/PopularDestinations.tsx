...
export default function PopularDestinations() {
    ...
    const nextSlide = () => {
        setCurrentIndex((prev) => (prev + itemsPerView >= destinations.length ? 0 : prev + 1))
    }

    const prevSlide = () => {
        setCurrentIndex((prev) => (prev === 0 ? Math.max(destinations.length - itemsPerView, 0) : prev - 1))
    }
                        ...
                        >
                            {destinations.map((destination) => (
                                <div
                                    ...
                                    onClick={() => goToPlaceList(destination.code, destination?.sigungu)}
                                >
